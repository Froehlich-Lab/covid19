...
p1<-ggplot(covid_countries, aes(x=date, y=log(cases_per_million+1), group=entity)) +
...
trade_data <- read_csv(here("data", "imports.csv")) %>% 
  rename(iso_a3 = ISO3c) %>%
...
p2 <- ggplot(data=bbox_df, aes(long,lat, group=group)) + geom_polygon(fill="aliceblue")+
  geom_polygon(data=countries_df, aes(x=long, y=lat, group=group, map_id=id, fill=(total_trade)))+
  theme_nothing()+
  theme(legend.position = "bottom",
        legend.text = element_text(size=8),
        legend.title = element_text(size=8),
        legend.box.spacing = unit(-1, "cm"))+
  scale_fill_viridis_c(direction=-1, option="A", na.value = "grey90" ,labels=c(2.5, 5, 7.5, 10), breaks=c(2.5e+08, 5e+08, 7.5e+08, 10e+08))+
  guides(fill=guide_colorbar(title.position = "top", title.hjust = 0.5, barwidth = 10, barheight = 0.5, units="cm"))+
  labs(fill=bquote(Total~trade~flows~(million~tonnes))) +
