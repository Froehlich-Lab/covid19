...
p2 <- ggplot(data=bbox_df, aes(long,lat, group=group)) + geom_polygon(fill="aliceblue")+
  ...
  guides(fill=guide_colorbar(title.position = "top", title.hjust = 1, barwidth = 10, barheight = 0.5, units="cm"))+
  labs(fill=bquote(Total~seafood~trade~with~China~(Tx10^6))) +
  ggsave(here("plots", paste0("seafood_trade_map_", Sys.Date(), ".tiff")), device="tiff", dpi=300, width=12, height=7, units="cm")
