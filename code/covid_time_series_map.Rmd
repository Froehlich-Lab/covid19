...
centroids.df$`coordinates(...)

coordinates(centroids.df)
...
(p2 <- ggplot(data=bbox_df, aes(long,lat, group=group)) + geom_polygon(fill="aliceblue")+
  ...
  scale_fill_viridis_c(direction=-1, option="D", na.value = "grey90",labels=c(2.5, 5, 7.5, 10), breaks=c(2.5e+08, 5e+08, 7.5e+08, 10e+08))
  ...
  labs(fill=bquote(Total~seafood~trade~with~China~(Tx10^6))) +
    geom_label(data = centroids.df, aes(x=))+
